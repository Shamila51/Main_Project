# -*- coding: utf-8 -*-
"""Employee_turn_over_prediction.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1gpGNWxgMR9Ozh9k3G2r9nbYDg9VcadHe
...
# prompt: save the random forest model to drive i need to load it using pickle
...
with open('/content/drive/MyDrive/Project/Code/model/random_forest_model.pkl', 'wb') as file:
  pickle.dump(rf, file)
...
# prompt: load model from /content/drive/MyDrive/Project/Code/model

import pickle

# Load the saved model
with open('/content/drive/MyDrive/Project/Code/model/random_forest_model.pkl', 'rb') as file:
...
# prompt: find proportion of result by  with  prediction 0 and prediction =1 as a dictionary combine this with result

def calculate_proportions(pred_df):

  total_predictions = len(pred_df)
  proportion_prediction_0 = len(pred_df[pred_df['predictions'] == 0]) / total_predictions
  proportion_prediction_1 = len(pred_df[pred_df['predictions'] == 1]) / total_predictions

  result_dict = {
      "Prediction_0": proportion_prediction_0,
      "Prediction_1": proportion_prediction_1
  }

  return result_dict

# Example usage (assuming pred_df is defined)
result_dict = calculate_proportions(pred_df)
...
#predictions1 = new.reset_index().rename(columns={"index": "Prediction_Label"}).to_dict(orient='records')

#print("Formatted Predictions for Anvil:", predictions1)  # Debugging
        #return predictions
...
def predict_turnover(file):
    """Predicts employee turnover based on uploaded CSV file"""
    try:
        if file.content_type != "text/csv":
            return [{"Error": "Invalid file format. Please upload a CSV file."}]

        # Read CSV file
        df_new = pd.read_csv(io.BytesIO(file.get_bytes()))


        # Print uploaded CSV columns for debugging
        print("Columns in uploaded CSV:", df_new.columns.tolist())
        df_new.head()
        ...
        df_copy = df_new.copy()  # Create a copy before renaming

        # Print renamed columns for debugging
        print("Renamed columns:", df_new.columns.tolist())

        # Ensure required columns exist before proceeding
        required_columns = ['satisfaction', 'evaluation', 'yearsAtCompany']
        missing_columns = [col for col in required_columns if col not in df_new.columns]

        if missing_columns:
            return [{"Error": f"Missing columns: {', '.join(missing_columns)}. Please check your CSV file."}]

        # Load Trained Model
        model_path = "/content/drive/MyDrive/Project/Code/model/random_forest_model.pkl"
        with open(model_path, 'rb') as file:
            loaded_model = pickle.load(file)

        # Ensure df_new has the same columns as training data
        missing_cols = set(X_train.columns) - set(df_new.columns)
        for col in missing_cols:
            df_new[col] = 0  # Fill missing columns with default values

        # Reorder columns to match training data
        df_new = df_new[X_train.columns]
        # Predict Turnover
        predictions_new = loaded_model.predict(df_new)
        print("Predictions:", predictions_new)  # Debugging print

        # Convert NumPy array to Pandas Series before mapping
        predictions_series = pd.Series(predictions_new, index=df_new.index)

        total_count = len(predictions_series)  # Total number of records
        #proportion_values = predictions_series['Prediction'].value_counts(normalize=True)
        proportion_values = predictions_series.value_counts(normalize=True)


        result = pd.DataFrame({
          'Prediction': predictions_series,
          'Prediction_Label': predictions_series.map({0: "❌ Not at Risk of Leaving", 1: "✅ At Risk of Leaving"}),
          'Proportion': predictions_series.map(proportion_values),  # Fixed mapping
          'Satisfaction': df_copy['satisfaction'],
          'Salary': df_copy['salary'] if 'salary' in df_copy.columns else "N/A",
          'Department': df_copy['department'] if 'department' in df_copy.columns else "N/A",
          'Evaluation': df_copy['evaluation'],
          'YearsAtCompany': df_copy['yearsAtCompany']
      })
        ...
        # Group by Prediction_Label and calculate summary statistics
        turnover_summary = result.groupby('Prediction_Label').mean(numeric_only=True).to_dict()

        # Convert summary stats to a structured format
        summary_df = pd.DataFrame(turnover_summary).reset_index().rename(columns={"index": "Prediction_Label"})

        # Convert DataFrame to a list of dictionaries for Anvil
        predictions = summary_df.to_dict(orient='records')

        print("Formatted Predictions for Anvil:", predictions)  # Debugging
        return predictions

    except Exception as e:
        return [{"Error": str(e)}]

anvil.server.connect("server_D5SYPUE2MAUBUP6D2IMRAJCY-64ZXXTZ7RGOISGAN")
